...
import ru.ke46138.besednik.account.ContactsManager;
import ru.ke46138.besednik.account.Utils;
import ru.ke46138.besednik.db.BaseDb;
import ru.ke46138.besednik.media.VxCard;
import ru.ke46138.besednik.widgets.LetterTileDrawable;
import ru.ke46138.besednik.widgets.OnlineDrawable;
import ru.ke46138.besednik.widgets.RoundImageDrawable;
import ru.ke46138.besednik.widgets.UrlLayerDrawable;
...
public class UiUtils {
    ...
    private static final Handler typingAnimationHandler = new Handler();
    private static String typingText = "";
    private static int typingDotCount = 0;
    private static boolean isTypingAnimating = false; // Флаг для проверки состояния анимации
    private static final long typingTIMEOUT_MS = 5000; // Тайм-аут в миллисекундах
    private static final Handler typingTimeoutHandler = new Handler(); // Handler для тайм-аута
    ...
    static Timer toolbarTypingIndicator(final Activity activity, Timer timer, int duration) {
        typingText = activity.getString(R.string.typing);
        if (timer != null) {
            timer.cancel();
        }
        ...
        if (!isTypingAnimating) {
            typingAnimationHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    // Обновляем текст с точками
                    StringBuilder animatedText = new StringBuilder(typingText);
                    for (int i = 0; i < typingDotCount; i++) {
                        animatedText.append(".");
                    }
                    toolbar.setSubtitle(animatedText.toString());

                    // Увеличиваем количество точек или сбрасываем его
                    typingDotCount = (typingDotCount + 1) % 4;

                    // Повторяем через 500 миллисекунд
                    typingAnimationHandler.postDelayed(this, 500);
                }
            }, 500);
        }
        isTypingAnimating = true;

        // Сбрасываем предыдущий тайм-аут
        typingTimeoutHandler.removeCallbacksAndMessages(null);

        // Устанавливаем новый тайм-аут
        typingTimeoutHandler.postDelayed(() -> {
            isTypingAnimating = false; // Сбрасываем флаг
            typingAnimationHandler.removeCallbacksAndMessages(null); // Удаляем все задачи анимации
            typingTimeoutHandler.removeCallbacksAndMessages(null); // Очищаем задачи тайм-аута
            toolbar.setSubtitle("");
        }, typingTIMEOUT_MS);
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                activity.runOnUiThread(() -> {
                    ...
                    typing.setAlpha(0);
                    isTypingAnimating = false; // Сбрасываем флаг
                    typingAnimationHandler.removeCallbacksAndMessages(null); // Удаляем все отложенные задачи
                    typingTimeoutHandler.removeCallbacksAndMessages(null); // Очищаем задачи тайм-аута
                    toolbar.setSubtitle("");
                });
            }
